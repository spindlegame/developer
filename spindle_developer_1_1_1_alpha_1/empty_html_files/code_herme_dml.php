...
function code_herme_insert(){
	...
	$data['symbols'] = makeSafe($_POST['symbols']);
...
function code_herme_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)){
		...
		$jsReadOnly .= "\tjQuery('#symbols').replaceWith('<div class=\"form-control-static\" id=\"symbols\">' + (jQuery('#symbols').val() || '') + '</div>');\n";
	...
	$parent_tables = array(  'biblio_author' => array('author', 'Author'), 'biblio_doc' => array('bibliography', 'Bibliography'), 'biblio_trascript' => array('transcript', 'Transcript'), 'biblio_token' => array('token_sequence', 'Token sequence'), 'class_im' => array('impression', 'Impression management'), 'class_nt' => array('noetictension', 'Noetic tension'));
	...
	$templateCode=str_replace('<%%UPLOADFILE(symbols)%%>', '', $templateCode);
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(symbols)%%>', htmlspecialchars($row['symbols'], ENT_QUOTES), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(symbols)%%>', urlencode($urow['symbols']), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(symbols)%%>', '', $templateCode);
		$templateCode=str_replace('<%%URLVALUE(symbols)%%>', urlencode(''), $templateCode);
