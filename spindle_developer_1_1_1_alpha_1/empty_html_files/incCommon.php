	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			...
			'storyboard' => array('Storyboard', '', 'table.gif'),
			...
			'class_bibliography_archive' => array('Archives', '', 'table.gif'),
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'code_herme' => "`code_herme`.`id` as 'id', IF(    CHAR_LENGTH(`biblio_author1`.`memberID`) || CHAR_LENGTH(`biblio_author1`.`last_name`), CONCAT_WS('',   `biblio_author1`.`memberID`, '  ', `biblio_author1`.`last_name`), '') as 'author', IF(    CHAR_LENGTH(`biblio_doc1`.`id`) || CHAR_LENGTH(`biblio_doc1`.`title`), CONCAT_WS('',   `biblio_doc1`.`id`, '  ', `biblio_doc1`.`title`), '') as 'bibliography', IF(    CHAR_LENGTH(`biblio_trascript1`.`trascriber_memberID`) || CHAR_LENGTH(`biblio_trascript1`.`transcript_title`), CONCAT_WS('',   `biblio_trascript1`.`trascriber_memberID`, ' - ', `biblio_trascript1`.`transcript_title`), '') as 'transcript', IF(    CHAR_LENGTH(`biblio_token1`.`token_sequence`), CONCAT_WS('',   `biblio_token1`.`token_sequence`), '') as 'token_sequence', IF(    CHAR_LENGTH(`biblio_token1`.`id`) || CHAR_LENGTH(`biblio_token1`.`token`), CONCAT_WS('',   `biblio_token1`.`id`, '  ', `biblio_token1`.`token`), '') as 'token', IF(    CHAR_LENGTH(`class_im1`.`impression`), CONCAT_WS('',   `class_im1`.`impression`), '') as 'impression', IF(    CHAR_LENGTH(`class_nt1`.`noetictension`), CONCAT_WS('',   `class_nt1`.`noetictension`), '') as 'noetictension', `code_herme`.`symbols` as 'symbols', `code_herme`.`freecode` as 'freecode'",
			'code_chrev_scenes' => "`code_chrev_scenes`.`id` as 'id', IF(    CHAR_LENGTH(`biblio_author1`.`memberID`), CONCAT_WS('',   `biblio_author1`.`memberID`), '') as 'author', IF(    CHAR_LENGTH(`biblio_doc1`.`id`) || CHAR_LENGTH(`biblio_doc1`.`title`), CONCAT_WS('',   `biblio_doc1`.`id`, '  ', `biblio_doc1`.`title`), '') as 'bibliography', IF(    CHAR_LENGTH(`biblio_trascript1`.`trascriber_memberID`) || CHAR_LENGTH(`biblio_trascript1`.`transcript_title`), CONCAT_WS('',   `biblio_trascript1`.`trascriber_memberID`, ' - ', `biblio_trascript1`.`transcript_title`), '') as 'transcript', IF(    CHAR_LENGTH(`biblio_token1`.`token_sequence`), CONCAT_WS('',   `biblio_token1`.`token_sequence`), '') as 'token_sequence', IF(    CHAR_LENGTH(`biblio_token1`.`token`), CONCAT_WS('',   `biblio_token1`.`token`, '  '), '') as 'token', IF(    CHAR_LENGTH(`biblio_author2`.`memberID`), CONCAT_WS('',   `biblio_author2`.`memberID`), '') as 'agent', IF(    CHAR_LENGTH(`code_invivo1`.`id`) || CHAR_LENGTH(`biblio_token3`.`token_sequence`), CONCAT_WS('',   `code_invivo1`.`id`, ' / ', `biblio_token3`.`token_sequence`), '') as 'invivo_code', IF(    CHAR_LENGTH(`code_invivo1`.`start_date`), CONCAT_WS('',   `code_invivo1`.`start_date`), '') as 'start_date', IF(    CHAR_LENGTH(`code_invivo1`.`end_date`), CONCAT_WS('',   `code_invivo1`.`end_date`), '') as 'end_date', IF(    CHAR_LENGTH(`code_invivo1`.`person`), CONCAT_WS('',   `code_invivo1`.`person`), '') as 'person', IF(    CHAR_LENGTH(`code_invivo1`.`place`), CONCAT_WS('',   `code_invivo1`.`place`), '') as 'place', IF(    CHAR_LENGTH(`code_invivo1`.`freecode`), CONCAT_WS('',   `code_invivo1`.`freecode`), '') as 'freecode', IF(    CHAR_LENGTH(`code_herme1`.`id`) || CHAR_LENGTH(`code_herme1`.`freecode`), CONCAT_WS('',   `code_herme1`.`id`, '/', `code_herme1`.`freecode`), '') as 'herme_code', IF(    CHAR_LENGTH(`class_im1`.`impression`), CONCAT_WS('',   `class_im1`.`impression`), '') as 'impression', IF(    CHAR_LENGTH(`class_nt1`.`noetictension`), CONCAT_WS('',   `class_nt1`.`noetictension`), '') as 'noetictension', IF(    CHAR_LENGTH(`code_herme1`.`symbols`), CONCAT_WS('',   `code_herme1`.`symbols`), '') as 'symbols', IF(    CHAR_LENGTH(`code_herme1`.`freecode`), CONCAT_WS('',   `code_herme1`.`freecode`), '') as 'comment', IF(    CHAR_LENGTH(`code_encounter_scenes1`.`scene`), CONCAT_WS('',   `code_encounter_scenes1`.`scene`), '') as 'scene'",
			...
			'class_bibliography_archive' => "`class_bibliography_archive`.`id` as 'id', `class_bibliography_archive`.`organization_abr` as 'organization_abr', `class_bibliography_archive`.`organization` as 'organization', `class_bibliography_archive`.`address_street` as 'address_street', `class_bibliography_archive`.`address_place` as 'address_place', `class_bibliography_archive`.`address_pc` as 'address_pc', `class_bibliography_archive`.`address_region` as 'address_region', `class_bibliography_archive`.`address_country` as 'address_country', `class_bibliography_archive`.`phone1` as 'phone1', `class_bibliography_archive`.`email1` as 'email1', `class_bibliography_archive`.`web1` as 'web1'",
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'code_chrev_scenes' => "`code_chrev_scenes` LEFT JOIN `biblio_author` as biblio_author1 ON `biblio_author1`.`id`=`code_chrev_scenes`.`author` LEFT JOIN `biblio_doc` as biblio_doc1 ON `biblio_doc1`.`id`=`code_chrev_scenes`.`bibliography` LEFT JOIN `biblio_trascript` as biblio_trascript1 ON `biblio_trascript1`.`id`=`code_chrev_scenes`.`transcript` LEFT JOIN `biblio_token` as biblio_token1 ON `biblio_token1`.`id`=`code_chrev_scenes`.`token` LEFT JOIN `biblio_author` as biblio_author2 ON `biblio_author2`.`id`=`code_chrev_scenes`.`agent` LEFT JOIN `code_invivo` as code_invivo1 ON `code_invivo1`.`id`=`code_chrev_scenes`.`start_date` LEFT JOIN `code_herme` as code_herme1 ON `code_herme1`.`id`=`code_chrev_scenes`.`herme_code` LEFT JOIN `code_encounter_scenes` as code_encounter_scenes1 ON `code_encounter_scenes1`.`id`=`code_chrev_scenes`.`scene` LEFT JOIN `biblio_token` as biblio_token3 ON `biblio_token3`.`id`=`code_invivo1`.`token_sequence` LEFT JOIN `class_im` as class_im1 ON `class_im1`.`id`=`code_herme1`.`impression` LEFT JOIN `class_nt` as class_nt1 ON `class_nt1`.`id`=`code_herme1`.`noetictension` ",
			...
			'class_bibliography_archive' => "`class_bibliography_archive` ",
