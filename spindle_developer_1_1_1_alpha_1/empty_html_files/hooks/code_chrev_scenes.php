<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'code_chrev_scenes' table. 
	...
	*/
	...
	function code_chrev_scenes_init(&$options, $memberInfo, &$args){
	...
	*/
	...
	function code_chrev_scenes_header($contentType, $memberInfo, &$args){
		$header='';
	...
	*/
	...
	function code_chrev_scenes_footer($contentType, $memberInfo, &$args){
		$footer='';
	 ...
	 *     $data['author'], $data['bibliography'], $data['transcript'], $data['token_sequence'], $data['token'], $data['agent'], $data['invivo_code'], $data['start_date'], $data['end_date'], $data['person'], $data['place'], $data['freecode'], $data['herme_code'], $data['impression'], $data['noetictension'], $data['symbols'], $data['comment'], $data['scene']
	...
	*/
	...
	function code_chrev_scenes_before_insert(&$data, $memberInfo, &$args){
	...
	*/
	...
	function code_chrev_scenes_after_insert($data, $memberInfo, &$args){
	 ...
	 * Note: if a field is set as read-only or hidden in detail view, it can't be modified through $data. You should use a direct SQL statement instead.
	 * For this table, the array items are: 
	 *     $data['id'], $data['author'], $data['bibliography'], $data['transcript'], $data['token_sequence'], $data['token'], $data['agent'], $data['invivo_code'], $data['start_date'], $data['end_date'], $data['person'], $data['place'], $data['freecode'], $data['herme_code'], $data['impression'], $data['noetictension'], $data['symbols'], $data['comment'], $data['scene']
	...
	*/
	...
	function code_chrev_scenes_before_update(&$data, $memberInfo, &$args){
	...
	*/
	...
	function code_chrev_scenes_after_update($data, $memberInfo, &$args){
	...
	*/
	...
	function code_chrev_scenes_before_delete($selectedID, &$skipChecks, $memberInfo, &$args){
	...
	*/
	...
	function code_chrev_scenes_after_delete($selectedID, $memberInfo, &$args){
	...
	*/
	...
	function code_chrev_scenes_dv($selectedID, $memberInfo, &$html, &$args){
	...
	*/
	...
	function code_chrev_scenes_csv($query, $memberInfo, &$args){
	...
	*/
	...
	function code_chrev_scenes_batch_actions(&$args){
