<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'biblio_doc' table. 
	...
	*/
	...
	function biblio_doc_init(&$options, $memberInfo, &$args){
	...
	*/
	...
	function biblio_doc_header($contentType, $memberInfo, &$args){
		$header='';
	...
	*/
	...
	function biblio_doc_footer($contentType, $memberInfo, &$args){
		$footer='';
	 ...
	 *     $data['img'], $data['author_name'], $data['author_id'], $data['type'], $data['genre'], $data['created'], $data['published'], $data['title'], $data['subtitle'], $data['publisher'], $data['location'], $data['citation'], $data['description'], $data['source'], $data['medium'], $data['language'], $data['format'], $data['subject'], $data['rights'], $data['rights_holder'], $data['data_evaluation'], $data['authority_record'], $data['authority_organization'], $data['archive'], $data['pdf_book'], $data['ext_source']
	...
	*/
	...
	function biblio_doc_before_insert(&$data, $memberInfo, &$args){
	...
	*/
	...
	function biblio_doc_after_insert($data, $memberInfo, &$args){
	 ...
	 * Note: if a field is set as read-only or hidden in detail view, it can't be modified through $data. You should use a direct SQL statement instead.
	 * For this table, the array items are: 
	 *     $data['id'], $data['img'], $data['author_name'], $data['author_id'], $data['type'], $data['genre'], $data['created'], $data['published'], $data['title'], $data['subtitle'], $data['publisher'], $data['location'], $data['citation'], $data['description'], $data['source'], $data['medium'], $data['language'], $data['format'], $data['subject'], $data['rights'], $data['rights_holder'], $data['data_evaluation'], $data['authority_record'], $data['authority_organization'], $data['archive'], $data['pdf_book'], $data['ext_source']
	...
	*/
	...
	function biblio_doc_before_update(&$data, $memberInfo, &$args){
	...
	*/
	...
	function biblio_doc_after_update($data, $memberInfo, &$args){
	...
	*/
	...
	function biblio_doc_before_delete($selectedID, &$skipChecks, $memberInfo, &$args){
	...
	*/
	...
	function biblio_doc_after_delete($selectedID, $memberInfo, &$args){
	...
	*/
	...
	function biblio_doc_dv($selectedID, $memberInfo, &$html, &$args){
	...
	*/
	...
	function biblio_doc_csv($query, $memberInfo, &$args){
	...
	*/
	...
	function biblio_doc_batch_actions(&$args){
