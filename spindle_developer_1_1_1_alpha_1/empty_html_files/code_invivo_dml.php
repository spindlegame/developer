...
// Data functions (insert, update, delete, form) for table code_invivo

// This script and data application were generated by AppGini 5.40
// Download AppGini for free from http://bigprof.com/appgini/download/

function code_invivo_insert(){
	global $Translation;

	if($_GET['insert_x']!=''){$_POST=$_GET;}

	// mm: can member insert record?
	$arrPerm=getTablePermissions('code_invivo');
	if(!$arrPerm[1]){
		return false;
	}
	...
	// hook: code_invivo_before_insert
	if(function_exists('code_invivo_before_insert')){
		$args=array();
		if(!code_invivo_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('insert into `code_invivo` set       `author`=' . (($data['author'] !== '' && $data['author'] !== NULL) ? "'{$data['author']}'" : 'NULL') . ', `bibliography`=' . (($data['bibliography'] !== '' && $data['bibliography'] !== NULL) ? "'{$data['bibliography']}'" : 'NULL') . ', `transcript`=' . (($data['transcript'] !== '' && $data['transcript'] !== NULL) ? "'{$data['transcript']}'" : 'NULL') . ', `token_sequence`=' . (($data['token_sequence'] !== '' && $data['token_sequence'] !== NULL) ? "'{$data['token_sequence']}'" : 'NULL') . ', `token`=' . (($data['token'] !== '' && $data['token'] !== NULL) ? "'{$data['token']}'" : 'NULL') . ', `invivo`=' . (($data['invivo'] !== '' && $data['invivo'] !== NULL) ? "'{$data['invivo']}'" : 'NULL') . ', `start_date`=' . (($data['start_date'] !== '' && $data['start_date'] !== NULL) ? "'{$data['start_date']}'" : 'NULL') . ', `end_date`=' . (($data['end_date'] !== '' && $data['end_date'] !== NULL) ? "'{$data['end_date']}'" : 'NULL') . ', `person`=' . (($data['person'] !== '' && $data['person'] !== NULL) ? "'{$data['person']}'" : 'NULL') . ', `place`=' . (($data['place'] !== '' && $data['place'] !== NULL) ? "'{$data['place']}'" : 'NULL') . ', `freecode`=' . (($data['freecode'] !== '' && $data['freecode'] !== NULL) ? "'{$data['freecode']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"code_invivo_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID=db_insert_id(db_link());

	// hook: code_invivo_after_insert
	if(function_exists('code_invivo_after_insert')){
		$res = sql("select * from `code_invivo` where `id`='" . makeSafe($recID) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID);
		$args=array();
		if(!code_invivo_after_insert($data, getMemberInfo(), $args)){ return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID); }
	}

	// mm: save ownership data
	sql("insert into membership_userrecords set tableName='code_invivo', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);

	return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID);
}

function code_invivo_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('code_invivo');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='code_invivo' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='code_invivo' and pkValue='$selected_id'");
	...
	// hook: code_invivo_before_delete
	if(function_exists('code_invivo_before_delete')){
		$args=array();
		if(!code_invivo_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	// child table: code_chrev_scenes
	$res = sql("select `id` from `code_invivo` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `code_chrev_scenes` where `start_date`='".addslashes($id[0])."'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "code_chrev_scenes", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='code_invivo_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='code_invivo_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}

	sql("delete from `code_invivo` where `id`='$selected_id'", $eo);

	// hook: code_invivo_after_delete
	if(function_exists('code_invivo_after_delete')){
		$args=array();
		code_invivo_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='code_invivo' and pkValue='$selected_id'", $eo);
}

function code_invivo_update($selected_id){
	global $Translation;
	...
	$arrPerm=getTablePermissions('code_invivo');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='code_invivo' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='code_invivo' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: code_invivo_before_update
	if(function_exists('code_invivo_before_update')){
		$args=array();
		if(!code_invivo_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `code_invivo` set       `author`=' . (($data['author'] !== '' && $data['author'] !== NULL) ? "'{$data['author']}'" : 'NULL') . ', `bibliography`=' . (($data['bibliography'] !== '' && $data['bibliography'] !== NULL) ? "'{$data['bibliography']}'" : 'NULL') . ', `transcript`=' . (($data['transcript'] !== '' && $data['transcript'] !== NULL) ? "'{$data['transcript']}'" : 'NULL') . ', `token_sequence`=' . (($data['token_sequence'] !== '' && $data['token_sequence'] !== NULL) ? "'{$data['token_sequence']}'" : 'NULL') . ', `token`=' . (($data['token'] !== '' && $data['token'] !== NULL) ? "'{$data['token']}'" : 'NULL') . ', `invivo`=' . (($data['invivo'] !== '' && $data['invivo'] !== NULL) ? "'{$data['invivo']}'" : 'NULL') . ', `start_date`=' . (($data['start_date'] !== '' && $data['start_date'] !== NULL) ? "'{$data['start_date']}'" : 'NULL') . ', `end_date`=' . (($data['end_date'] !== '' && $data['end_date'] !== NULL) ? "'{$data['end_date']}'" : 'NULL') . ', `person`=' . (($data['person'] !== '' && $data['person'] !== NULL) ? "'{$data['person']}'" : 'NULL') . ', `place`=' . (($data['place'] !== '' && $data['place'] !== NULL) ? "'{$data['place']}'" : 'NULL') . ', `freecode`=' . (($data['freecode'] !== '' && $data['freecode'] !== NULL) ? "'{$data['freecode']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="code_invivo_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: code_invivo_after_update
	if(function_exists('code_invivo_after_update')){
		$res = sql("SELECT * FROM `code_invivo` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!code_invivo_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='code_invivo' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function code_invivo_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	if($selected_id){
		...
		$res = sql("select * from `code_invivo` where `id`='".makeSafe($selected_id)."'", $eo);
	...
	$combo_token_sequence->HTML = '<span id="token_sequence-container' . $rnd1 . '"></span><input type="hidden" name="token_sequence" id="token_sequence' . $rnd1 . '" value="' . htmlspecialchars($combo_token_sequence->SelectedData) . '">';
	$combo_token_sequence->MatchText = '<span id="token_sequence-container-readonly' . $rnd1 . '"></span><input type="hidden" name="token_sequence" id="token_sequence' . $rnd1 . '" value="' . htmlspecialchars($combo_token_sequence->SelectedData) . '">';
	...
	<script>
			...
			jQuery("#author-container__RAND__").select2({
				...
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: current_author__RAND__.value, t: 'code_invivo', f: 'author' }
					}).done(function(resp){
						c({
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p: page, t: 'code_invivo', f: 'author' }; },
			...
			jQuery("#bibliography-container__RAND__").select2({
				...
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, id: current_bibliography__RAND__.value, t: 'code_invivo', f: 'bibliography' }
					}).done(function(resp){
						c({
							id: resp.results[0].id,
							text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { filterer_author: filterer_author, s: term, p: page, t: 'code_invivo', f: 'bibliography' }; },
					results: function(resp, page){ return resp; }
				}
			}).on('change', function(e){
		...
		<?php }else{ ?>
			...
			jQuery.ajax({
				url: 'ajax_combo.php',
				dataType: 'json',
				data: { id: current_bibliography__RAND__.value, t: 'code_invivo', f: 'bibliography' }
			}).done(function(resp){
			...
			jQuery("#transcript-container__RAND__").select2({
				...
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, id: current_transcript__RAND__.value, t: 'code_invivo', f: 'transcript' }
					}).done(function(resp){
						c({
							id: resp.results[0].id,
							text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, s: term, p: page, t: 'code_invivo', f: 'transcript' }; },
					results: function(resp, page){ return resp; }
				}
			}).on('change', function(e){
		...
		<?php }else{ ?>
			...
			jQuery.ajax({
				url: 'ajax_combo.php',
				dataType: 'json',
				data: { id: current_transcript__RAND__.value, t: 'code_invivo', f: 'transcript' }
			}).done(function(resp){
			...
			jQuery("#token_sequence-container__RAND__").select2({
				...
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, id: current_token_sequence__RAND__.value, t: 'code_invivo', f: 'token_sequence' }
					}).done(function(resp){
						c({
							id: resp.results[0].id,
							text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, s: term, p: page, t: 'code_invivo', f: 'token_sequence' }; },
					results: function(resp, page){ return resp; }
				}
			}).on('change', function(e){
		...
		<?php }else{ ?>
			...
			jQuery.ajax({
				url: 'ajax_combo.php',
				dataType: 'json',
				data: { id: current_token_sequence__RAND__.value, t: 'code_invivo', f: 'token_sequence' }
			}).done(function(resp){
	...
	if($dvprint){
		$templateCode = @file_get_contents('./templates/code_invivo_templateDVP.html');
	}else{
		$templateCode = @file_get_contents('./templates/code_invivo_templateDV.html');
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode=str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return code_invivo_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode=str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return code_invivo_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode=str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return code_invivo_validateData();"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$parent_tables = array(  'biblio_author' => array('author', 'Author'), 'biblio_doc' => array('bibliography', 'Bibliography'), 'biblio_trascript' => array('transcript', 'Transcript'), 'biblio_token' => array('token_sequence', 'Token sequence'));
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(invivo)%%>', htmlspecialchars($row['invivo'], ENT_QUOTES), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(end_date)%%>', htmlspecialchars($row['end_date'], ENT_QUOTES), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(end_date)%%>', urlencode($urow['end_date']), $templateCode);
		$templateCode=str_replace('<%%VALUE(person)%%>', htmlspecialchars($row['person'], ENT_QUOTES), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(person)%%>', urlencode($urow['person']), $templateCode);
		$templateCode=str_replace('<%%VALUE(place)%%>', htmlspecialchars($row['place'], ENT_QUOTES), $templateCode);
	...
	if($dvprint){
		$templateCode.="\t\t\t'code_invivo_autofill.php?rnd1=$rnd1&mfk=token_sequence&id='+encodeURIComponent('".addslashes($row['token_sequence'])."'),\n";
		$templateCode.="\t\t\t{encoding: 'UTF-8', method: 'get'}\n";
	}else{
		$templateCode.="\t\t\t'code_invivo_autofill.php?rnd1=$rnd1&mfk=token_sequence&id=' + encodeURIComponent(current_token_sequence{$rnd1}.value),\n";
	...
	// hook: code_invivo_dv
	if(function_exists('code_invivo_dv')){
		$args=array();
		code_invivo_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
