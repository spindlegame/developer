...
function class_dramatica_issue_insert(){
	global $Translation;
	...
	if(function_exists('class_dramatica_issue_before_insert')){
		$args=array();
		if(!class_dramatica_issue_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('insert into `class_dramatica_issue` set       `domain`=' . (($data['domain'] !== '' && $data['domain'] !== NULL) ? "'{$data['domain']}'" : 'NULL') . ', `concern`=' . (($data['concern'] !== '' && $data['concern'] !== NULL) ? "'{$data['concern']}'" : 'NULL') . ', `issue`=' . (($data['issue'] !== '' && $data['issue'] !== NULL) ? "'{$data['issue']}'" : 'NULL') . ', `description`=' . (($data['description'] !== '' && $data['description'] !== NULL) ? "'{$data['description']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"class_dramatica_issue_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	if(function_exists('class_dramatica_issue_after_insert')){
		$res = sql("select * from `class_dramatica_issue` where `id`='" . makeSafe($recID) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID);
		$args=array();
		if(!class_dramatica_issue_after_insert($data, getMemberInfo(), $args)){ return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID); }
	}

	// mm: save ownership data
	sql("insert into membership_userrecords set tableName='class_dramatica_issue', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);
...
function class_dramatica_issue_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('class_dramatica_issue');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='class_dramatica_issue' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='class_dramatica_issue' and pkValue='$selected_id'");
	...
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storypoints_static` where `issue`='".addslashes($id[0])."'", $eo);
	...
	$res = sql("select `id` from `class_dramatica_issue` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storypoints_static` where `catalyst`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `class_dramatica_issue` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storypoints_static` where `inhibitor`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `class_dramatica_issue` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storyweaving_scenes` where `issue`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `class_dramatica_issue` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `class_dramatica_themes` where `issue`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function class_dramatica_issue_update($selected_id){
	global $Translation;
	...
	if(function_exists('class_dramatica_issue_before_update')){
		$args=array();
		if(!class_dramatica_issue_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `class_dramatica_issue` set       `domain`=' . (($data['domain'] !== '' && $data['domain'] !== NULL) ? "'{$data['domain']}'" : 'NULL') . ', `concern`=' . (($data['concern'] !== '' && $data['concern'] !== NULL) ? "'{$data['concern']}'" : 'NULL') . ', `issue`=' . (($data['issue'] !== '' && $data['issue'] !== NULL) ? "'{$data['issue']}'" : 'NULL') . ', `description`=' . (($data['description'] !== '' && $data['description'] !== NULL) ? "'{$data['description']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function class_dramatica_issue_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	$combo_domain->HTML = '<span id="domain-container' . $rnd1 . '"></span><input type="hidden" name="domain" id="domain' . $rnd1 . '" value="' . htmlspecialchars($combo_domain->SelectedData) . '">';
	$combo_domain->MatchText = '<span id="domain-container-readonly' . $rnd1 . '"></span><input type="hidden" name="domain" id="domain' . $rnd1 . '" value="' . htmlspecialchars($combo_domain->SelectedData) . '">';
	...
	<script>
			...
			jQuery("#domain-container__RAND__").select2({
				...
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: current_domain__RAND__.value, t: 'class_dramatica_issue', f: 'domain' }
					}).done(function(resp){
						c({
			...
			jQuery("#concern-container__RAND__").select2({
				...
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_domain: filterer_domain, id: current_concern__RAND__.value, t: 'class_dramatica_issue', f: 'concern' }
					}).done(function(resp){
						c({
							id: resp.results[0].id,
							text: resp.results[0].text
		...
		<?php }else{ ?>
			...
			jQuery.ajax({
				url: 'ajax_combo.php',
				dataType: 'json',
				data: { id: current_concern__RAND__.value, t: 'class_dramatica_issue', f: 'concern' }
			}).done(function(resp){
	...
	if($dvprint){
		$templateCode = @file_get_contents('./templates/class_dramatica_issue_templateDVP.html');
	}else{
		$templateCode = @file_get_contents('./templates/class_dramatica_issue_templateDV.html');
	}
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode=str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return class_dramatica_issue_validateData();"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$parent_tables = array(  'class_dramatica_domain' => array('domain', 'domain'), 'class_dramatica_concern' => array('concern', 'concern'));
